...
RollingArchives.prototype.scrollTo = function ( elements, direction, next ) {
	...
	theBrowserWindow.animate( { scrollTop: nextElementPos }, (K2.Animations ? 150 : 0), 'easeOutExpo', function () {
...
RollingArchives.prototype.assignHotkeys = function () {
	...
	jQuery( document ).bind( 'keydown.hotkeys', 'J', function () {
		RA.scrollTo( RA.posts, 1 )
	} );

	// K: Scroll to previous post
	jQuery( document ).bind( 'keydown.hotkeys', 'K', function () {
		RA.scrollTo( RA.posts, -1 )
	} );

	// Enter: Go to selected post
	jQuery( document ).bind( 'keydown.hotkeys', 'Return', function () {
		if ( jQuery( RA.nextElement ).length > 0 ) {
			jQuery( RA.nextElement ).stop( true, true ).effect( "highlight", {color: '#eee'}, 150 ).children( '.entry-title a' ).click()
		}
	} );

	// K: Scroll to previous post
	jQuery( document ).bind( 'keydown.hotkeys', 'E', function () {
		if ( jQuery( '.selected' ).length > 0 ) {
			jQuery( '.selected a.post-edit-link' ).click();
			RA.flashElement( '.selected a.post-edit-link' )
		}
	} );

	// Esc: Deactivate selected post
	jQuery( document ).bind( 'keydown.hotkeys', 'Esc', function () {
		jQuery( window ).unbind( 'scroll.scrolldetector' );
	...
	// H: Go back to page 1  
	jQuery( document ).bind( 'keydown.hotkeys', 'H', function () {
		RA.gotoPage( 1 )
	} );

	// T: Trim, or remove .entry-content  
	jQuery( document ).bind( 'keydown.hotkeys', 'T', function () {
		jQuery( '#texttrimmer div:visible' ).click()
	} );

	// Left Arrow: Previous Page
	jQuery( document ).bind( 'keydown.hotkeys', 'Left', function () {
		RA.pageSlider.setValueBy( -1 )
	} );

	// Right Arrow: Next Page
	jQuery( document ).bind( 'keydown.hotkeys', 'Right', function () {
		RA.pageSlider.setValueBy( 1 )
	} );
};